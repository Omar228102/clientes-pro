...
const DEFAULT_GASTO_CATS = ["Alquiler","Servicios","Sueldos","Materiales","Viaticos","Impuestos","Otro"];
...
function printList({ title, company, headers, rows, totals }) {
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
    *{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;color:#111;padding:32px;}
    h1{font-size:20px;font-weight:800;margin-bottom:4px;}.sub{font-size:12px;color:#64748b;margin-bottom:20px;}
    ...
    <h1>${title}</h1>
    <div class="sub">${company&&company.name?company.name:""} ${company&&company.cuit?"&middot; CUIT "+company.cuit:""} &middot; ${new Date().toLocaleDateString("es-AR")}</div>
    <table>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080d1a",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      <div style={{textAlign:"center",padding:40}}>
        ...
        <p style={{color:"#475569",fontSize:15,margin:"0 0 40px"}}>Gestion de clientes y recibos profesional</p>
  ...
  return (
    <div style={S.overlay}>
      <div style={{...S.modal,maxWidth:500}}>
        <div style={S.modalHead}><span style={S.modalTitle}>Datos de la Empresa</span><button onClick={onClose} style={S.xBtn}>X</button></div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[["Nombre / Razon Social","name","text","1/-1"],["CUIT","cuit","text",""],["Telefono","phone","tel",""],["Email","email","email",""],["Direccion","address","text","1/-1"],["Sitio web","web","text",""],["Info adicional","extra","text","1/-1"]].map(([l,k,t,gc])=>(
            <div key={k} style={{...S.field,gridColumn:gc||"auto"}}>
              <label style={S.lbl}>{l}</label>
              <input type={t} value={f[k]||""} onChange={set(k)} style={S.inp}/>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={S.overlay}>
      <div style={{...S.modal,maxWidth:460}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 100px 36px",padding:"6px 0",marginBottom:6,gap:8}}>
        ...
        <div style={{marginBottom:14}}>
          {list.map((c,i)=>(
            <div key={c} style={{display:"grid",gridTemplateColumns:"1fr 100px 36px",gap:8,alignItems:"center",padding:"7px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        ...
        <div style={{fontSize:11,color:"#475569",marginBottom:14,padding:"6px 10px",background:"rgba(255,255,255,0.03)",borderRadius:6}}>
          <span style={{color:"#6ee7b7"}}>Gravados</span> se incluyen en el analisis. <span style={{color:"#fde68a"}}>No gravados</span> quedan fuera.
        </div>
  ...
  return (
    <div style={{...S.card,marginBottom:14}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        ...
        <div style={{...S.field,gridColumn:"1/-1"}}><label style={S.lbl}>Descripcion / Observaciones</label>
          <input value={f.description} onChange={set("description")} placeholder="Ej. Honorarios enero 2026" style={S.inp}/></div>
      ...
      <div style={{display:"flex",gap:8,marginTop:10,justifyContent:"flex-end"}}>
        ...
        <button onClick={()=>{if(!f.description||!f.amount)return alert("Completa descripcion y monto.");onAdd({...f,amount:parseFloat(f.amount),id:uid()});}} style={{...S.btn,...S.btnPrimary,padding:"7px 14px",fontSize:13}}>+ Agregar</button>
      </div>
    </div>
  );
}
...
function ClientForm({ initial, onSave, onCancel }) {
  ...
  const consultarARCA = async()=>{
    ...
    if(c.length!==11)return setArcaMsg("CUIT invalido");
    setArcaLoading(true);setArcaMsg("Consultando...");
    try{const r=await fetch("https://afip.tangofactura.com/Rest/GetContribuyenteFull?cuit="+c);const d=await r.json();if(d&&d.Contribuyente&&d.Contribuyente.nombre){setF(p=>({...p,name:d.Contribuyente.nombre,condicionFiscal:d.Contribuyente.categoriasMonotributo&&d.Contribuyente.categoriasMonotributo.length>0?"Monotributista":"Responsable Inscripto"}));setArcaMsg("OK: "+d.Contribuyente.nombre);}else setArcaMsg("No encontrado");}catch{setArcaMsg("Error de conexion");}
    setArcaLoading(false);
  };
  return (
    <div style={{...S.card,maxWidth:600,margin:"0 auto"}}>
      <h2 style={{margin:"0 0 18px",fontSize:18,fontWeight:800,color:"#f1f5f9"}}>{initial?"Editar Cliente":"Nuevo Cliente"}</h2>
      <div style={{...S.field,marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={consultarARCA} disabled={arcaLoading} style={{...S.btn,...S.btnPrimary,padding:"8px 12px",fontSize:12,opacity:arcaLoading?0.6:1}}>{arcaLoading?"...":"Consultar ARCA"}</button>
        </div>
        {arcaMsg&&<div style={{fontSize:11,marginTop:5,padding:"4px 8px",borderRadius:6,background:arcaMsg.startsWith("OK")?"rgba(110,231,183,0.1)":"rgba(252,165,165,0.1)",color:arcaMsg.startsWith("OK")?"#6ee7b7":"#fca5a5"}}>{arcaMsg}</div>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div style={{...S.field,gridColumn:"1/-1"}}><label style={S.lbl}>Nombre / Razon Social *</label><input value={f.name||""} onChange={set("name")} placeholder="Nombre del cliente" style={S.inp}/></div>
        <div style={S.field}><label style={S.lbl}>Condicion fiscal</label>
        ...
        <div style={S.field}><label style={S.lbl}>Telefono</label><input type="tel" value={f.phone||""} onChange={set("phone")} placeholder="11 1234-5678" style={S.inp}/></div>
...
function CuentaCorriente({ client, concepts, company, onBack, onUpdate, onSaveRecibo }) {
  ...
  const deleteItem = id=>{if(window.confirm("Eliminar item?"))onUpdate({...client,items:items.filter(i=>i.id!==id)});};
  ...
  return (
    <div>
      ...
      <div style={{...S.card,padding:0,overflow:"hidden"}}>
        ...
        {filtered.map((item,idx)=>(
          <div key={item.id} style={{display:"grid",gridTemplateColumns:"100px 2fr 100px 100px 110px 36px",padding:"9px 14px",borderBottom:"1px solid rgba(255,255,255,0.03)",background:idx%2===0?"rgba(15,23,42,0.2)":"transparent",alignItems:"center"}}>
            ...
            <button onClick={()=>deleteItem(item.id)} style={{...S.iconBtn,color:"#fca5a5",fontSize:12}}>X</button>
          </div>
        ))}
        {filtered.length>0&&(
      ...
      {showRecibo&&<ReciboModal client={client} company={company} concepts={concepts} onClose={()=>setShowRecibo(false)} onSaveRecibo={onSaveRecibo}/>}
  ...
  return (
    <div style={S.overlay}>
      <div style={{...S.modal,maxWidth:440}}>
        <div style={S.modalHead}><span style={S.modalTitle}>Copiar items de un mes a otro</span><button onClick={onClose} style={S.xBtn}>X</button></div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
          <div style={S.field}><label style={S.lbl}>Mes origen</label>
            <select value={fromMonth} onChange={e=>setFromMonth(Number(e.target.value))} style={S.inp}>{MONTHS.map((m,i)=><option key={i} value={i}>{m}</option>)}</select></div>
          <div style={S.field}><label style={S.lbl}>Anio origen</label>
            <select value={fromYear} onChange={e=>setFromYear(Number(e.target.value))} style={S.inp}>{[2024,2025,2026,2027].map(y=><option key={y}>{y}</option>)}</select></div>
          ...
          <div style={S.field}><label style={S.lbl}>Anio destino</label>
        ...
        <div style={{background:"rgba(15,23,42,0.5)",borderRadius:8,padding:12,marginBottom:16}}>
          <div style={{fontSize:11,color:"#64748b",marginBottom:8,fontWeight:700}}>Items a copiar ({preview.length})</div>
          {preview.length===0?<div style={{fontSize:12,color:"#334155"}}>Sin items en ese mes</div>:
            preview.map(i=><div key={i.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"3px 0"}}><span style={{color:"#94a3b8"}}>{i.concept} — {i.description}</span><span style={{color:"#6ee7b7",fontWeight:700}}>{fmt(i.amount)}</span></div>)}
        </div>
        <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
          <button onClick={onClose} style={{...S.btn,...S.btnGhost}}>Cancelar</button>
          <button onClick={handleCopy} disabled={!preview.length} style={{...S.btn,...S.btnPrimary}}>Copiar {preview.length} items</button>
        </div>
      </div>
    </div>
  );
...
function ReciboModal({ client, company, concepts, onClose, onSaveRecibo }) {
  ...
  const addExtra = ()=>{
    ...
    setExtraItems(p=>[...p,item]);setSelected(s=>[...s,item.id]);
    setEf({concept:concepts[0]||"Honorarios",description:"",amount:""});setAddingExtra(false);
  };
  const handleEmit = async(action)=>{
    if(!chosen.length)return alert("Selecciona al menos un item.");
    const recibo={id:uid(),folio,date:nowDate.toISOString().slice(0,10),month:nowDate.getMonth(),year:nowDate.getFullYear(),clientId:client.id,clientName:client.name,clientCuit:client.cuit||"",items:chosen,total,status:"pendiente"};
    ...
    }else if(action==="copy"){navigator.clipboard.writeText("Recibo "+folio+"\n"+chosen.map(i=>i.concept+": "+fmt(i.amount)).join("\n")+"\nTOTAL: "+fmt(total));alert("Copiado!");}
  };
  return (
    <div style={S.overlay}>
      <div style={{...S.modal,maxWidth:540}}>
        <div style={S.modalHead}><span style={S.modalTitle}>Emitir Recibo</span><button onClick={onClose} style={S.xBtn}>X</button></div>
        ...
        <p style={{fontSize:12,color:"#64748b",margin:"0 0 10px"}}>Selecciona los items a incluir:</p>
        <div style={{maxHeight:160,overflowY:"auto",marginBottom:10}}>
          {(client.items||[]).length===0&&<div style={{color:"#334155",fontSize:12,padding:"6px 0"}}>Sin items cargados.</div>}
          {(client.items||[]).map(item=>(
            <label key={item.id} style={S.checkRow}>
              ...
              <span style={{...S.badge,...(item.status==="pagado"?S.bPaid:S.bPend),fontSize:10,marginRight:8}}>{item.status==="pagado"?"Cobrado":"Pend."}</span>
        ...
        {extraItems.length>0&&extraItems.map(item=>(
          <label key={item.id} style={S.checkRow}>
            ...
            <button onClick={()=>setExtraItems(p=>p.filter(x=>x.id!==item.id))} style={{...S.iconBtn,color:"#fca5a5",fontSize:11}}>X</button>
          </label>
        ))}
        {addingExtra?(
          <div style={{background:"rgba(15,23,42,0.6)",borderRadius:8,padding:12,marginBottom:10}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
              <div style={S.field}><label style={S.lbl}>Concepto</label><select value={ef.concept} onChange={e=>setEf({...ef,concept:e.target.value})} style={S.inp}>{concepts.map(c=><option key={c}>{c}</option>)}</select></div>
              <div style={S.field}><label style={S.lbl}>Monto</label><input type="number" value={ef.amount} onChange={e=>setEf({...ef,amount:e.target.value})} placeholder="0.00" style={S.inp}/></div>
              <div style={{...S.field,gridColumn:"1/-1"}}><label style={S.lbl}>Descripcion</label><input value={ef.description} onChange={e=>setEf({...ef,description:e.target.value})} placeholder="Descripcion" style={S.inp}/></div>
            </div>
        ...
        ):<button onClick={()=>setAddingExtra(true)} style={{...S.btn,...S.btnGhost,width:"100%",marginBottom:12,fontSize:12,padding:"7px"}}>+ Agregar item al recibo</button>}
...
function ReciboEditor({ recibo, clients, concepts, company, onSave, onClose }) {
  const isNew = !recibo;
  const [clientId,setClientId] = useState(recibo&&recibo.clientId||"");
  const [items,setItems] = useState(recibo&&recibo.items||[]);
  const [date,setDate] = useState(recibo&&recibo.date||today());
  const [status,setStatus] = useState(recibo&&recibo.status||"pendiente");
  const [addingItem,setAddingItem] = useState(false);
  const [ef,setEf] = useState({concept:concepts[0]||"Honorarios",description:"",amount:""});
  const [clientSearch,setClientSearch] = useState("");
  const client = clients.find(c=>c.id===clientId);
  const total = items.reduce((s,i)=>s+(i.amount||0),0);
  const folio = recibo&&recibo.folio||"R-"+Date.now().toString().slice(-6);
  const addItem = ()=>{
    if(!ef.description||!ef.amount)return alert("Completa descripcion y monto.");
    setItems(p=>[...p,{...ef,amount:parseFloat(ef.amount),id:uid(),status,date}]);
    setEf({concept:concepts[0]||"Honorarios",description:"",amount:""});setAddingItem(false);
  };
  ...
  return (
    <div style={S.overlay}>
      <div style={{...S.modal,maxWidth:620}}>
        <div style={S.modalHead}><span style={S.modalTitle}>{isNew?"Nuevo Recibo":"Editar Recibo"}</span><button onClick={onClose} style={S.xBtn}>X</button></div>
        <div style={{...S.field,marginBottom:14}}>
          ...
          {!clientId?(
              ...
              <div style={{maxHeight:180,overflowY:"auto",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,overflow:"hidden"}}>
                ...
                {filteredClients.map(c=>(
                  <div key={c.id} onClick={()=>setClientId(c.id)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 12px",borderBottom:"1px solid rgba(255,255,255,0.04)",cursor:"pointer",background:"rgba(15,23,42,0.4)"}}>
                    <div><div style={{fontWeight:700,fontSize:13,color:"#f1f5f9"}}>{c.name}</div><div style={{fontSize:10,color:"#475569"}}>{c.cuit||""}{c.condicionFiscal?" · "+c.condicionFiscal:""}</div></div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,background:"rgba(110,231,183,0.08)",border:"1px solid rgba(110,231,183,0.2)",borderRadius:8,padding:"10px 14px"}}>
              <div style={{flex:1}}><div style={{fontWeight:700,color:"#f1f5f9"}}>{client&&client.name}</div><div style={{fontSize:11,color:"#475569"}}>{client&&client.cuit||""}{client&&client.condicionFiscal?" · "+client.condicionFiscal:""}</div></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
          <div style={S.field}><label style={S.lbl}>Fecha</label><input type="date" value={date} onChange={e=>setDate(e.target.value)} style={S.inp}/></div>
          <div style={S.field}>
            <label style={S.lbl}>Estado</label>
            <div style={{display:"flex",gap:8,marginTop:2}}>
              <button onClick={()=>setStatus("pendiente")} style={{...S.btn,flex:1,padding:"8px",fontSize:12,...(status==="pendiente"?{background:"rgba(252,165,165,0.2)",color:"#fca5a5",border:"1px solid rgba(252,165,165,0.4)"}:S.btnGhost)}}>Pendiente</button>
              <button onClick={()=>setStatus("pagado")} style={{...S.btn,flex:1,padding:"8px",fontSize:12,...(status==="pagado"?{background:"rgba(110,231,183,0.2)",color:"#6ee7b7",border:"1px solid rgba(110,231,183,0.4)"}:S.btnGhost)}}>Cobrado</button>
            </div>
          </div>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <label style={S.lbl}>Items del recibo</label>
          <button onClick={()=>setAddingItem(!addingItem)} style={{...S.btn,...S.btnPrimary,padding:"5px 12px",fontSize:12}}>{addingItem?"Cancelar":"+ Agregar item"}</button>
        </div>
        {addingItem&&(
          <div style={{background:"rgba(15,23,42,0.6)",borderRadius:8,padding:12,marginBottom:10}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
              <div style={S.field}><label style={S.lbl}>Concepto</label><select value={ef.concept} onChange={e=>setEf({...ef,concept:e.target.value})} style={S.inp}>{concepts.map(c=><option key={c}>{c}</option>)}</select></div>
              <div style={S.field}><label style={S.lbl}>Monto ($)</label><input type="number" value={ef.amount} onChange={e=>setEf({...ef,amount:e.target.value})} placeholder="0.00" style={S.inp}/></div>
              <div style={{...S.field,gridColumn:"1/-1"}}><label style={S.lbl}>Descripcion / Observaciones</label><input value={ef.description} onChange={e=>setEf({...ef,description:e.target.value})} onKeyDown={e=>e.key==="Enter"&&addItem()} placeholder="Ej. Honorarios enero 2026" style={S.inp}/></div>
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
        ...
        <div style={{border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,overflow:"hidden",marginBottom:12}}>
          {items.length===0&&<div style={{padding:16,textAlign:"center",color:"#334155",fontSize:12}}>Sin items. Agrega al menos uno.</div>}
          {items.map((item,idx)=>(
            <div key={item.id} style={{display:"grid",gridTemplateColumns:"120px 1fr 110px 36px",gap:8,padding:"8px 12px",borderBottom:"1px solid rgba(255,255,255,0.04)",alignItems:"center",background:idx%2===0?"rgba(15,23,42,0.3)":"transparent"}}>
              <select value={item.concept} onChange={e=>updateItem(item.id,"concept",e.target.value)} style={{...S.inp,fontSize:11,padding:"4px 6px"}}>{concepts.map(c=><option key={c}>{c}</option>)}</select>
              ...
              <button onClick={()=>removeItem(item.id)} style={{...S.iconBtn,color:"#fca5a5",fontSize:13}}>X</button>
            </div>
          ))}
          {items.length>0&&(
...
function RecibosView({ recibos, clients, company, concepts, onDeleteRecibo, onSaveRecibo, onUpdateClient }) {
  ...
  const handleCopyAll = async()=>{
    ...
    for(const r of filtered){
      ...
      const nr = {...r,id:uid(),folio:"R-"+Date.now().toString().slice(-6)+"-"+Math.random().toString(36).slice(2,5),date:newDate,month:toMonth,year:toYear,items:r.items.map(i=>({...i,id:uid(),status:"pendiente",date:newDate}))};
      await onSaveRecibo(nr);
    }
    setShowCopyAll(false);
    alert("Recibos copiados a "+MONTHS[toMonth]+" "+toYear);
  };
  ...
  const handleCopyText = ()=>{
    const text = activeSet.map(r=>
      "Recibo "+r.folio+" | "+r.date+"\nCliente: "+r.clientName+(r.clientCuit?" | CUIT: "+r.clientCuit:"")+"\n"+
    ...
    alert(activeSet.length+" recibo(s) copiado(s) al portapapeles");
  };
  ...
  return (
    <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"9px 14px",background:"rgba(30,41,59,0.8)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,marginBottom:10,flexWrap:"wrap"}}>
          <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",userSelect:"none"}}>
            ...
            <span style={{fontSize:12,color:"#94a3b8"}}>
              {selected.length===0?"Seleccionar todos":selected.length===filtered.length?"Todos":selected.length+" de "+filtered.length}
            </span>
          ...
          <div style={{marginLeft:"auto",display:"flex",gap:8,flexWrap:"wrap"}}>
            <button onClick={handlePrintSelected} style={{...S.btn,...S.btnGhost,padding:"6px 11px",fontSize:12}}>Imprimir {selected.length>0?"("+selected.length+")":"todos"}</button>
            <button onClick={handleCopyText} style={{...S.btn,...S.btnGhost,padding:"6px 11px",fontSize:12}}>Copiar {selected.length>0?"("+selected.length+")":"todos"}</button>
            {selected.length>0&&<button onClick={()=>selectedRecibos.forEach(r=>generatePDF({name:r.clientName,cuit:r.clientCuit},company,r.items,r.folio,r.status))} style={{...S.btn,background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",padding:"6px 11px",fontSize:12}}>PDF ({selected.length})</button>}
            {selected.length>0&&<button onClick={()=>setSelected([])} style={{...S.iconBtn,color:"#64748b",fontSize:12}}>X</button>}
          </div>
        </div>
      )}
      <div style={{...S.card,padding:0,overflow:"hidden"}}>
        <div style={{display:"grid",gridTemplateColumns:"36px 95px 1fr 1fr 110px 175px",padding:"8px 14px",background:"rgba(15,23,42,0.8)",borderBottom:"1px solid rgba(255,255,255,0.06)",alignItems:"center"}}>
          ...
          {["Folio","Cliente","Items","Total","Acciones"].map(h=><div key={h} style={{fontSize:10,color:"#475569",textTransform:"uppercase",letterSpacing:"0.06em",fontWeight:700}}>{h}</div>)}
        </div>
        {filtered.length===0&&<div style={S.empty}>Sin recibos en {MONTHS[filterMonth]} {filterYear}<br/><span style={{fontSize:12,color:"#334155"}}>Crea uno con Nuevo Recibo</span></div>}
        {filtered.map((r,idx)=>(
          <div key={r.id} style={{display:"grid",gridTemplateColumns:"36px 95px 1fr 1fr 110px 175px",padding:"10px 14px",borderBottom:"1px solid rgba(255,255,255,0.03)",background:selected.includes(r.id)?"rgba(110,231,183,0.06)":idx%2===0?"rgba(15,23,42,0.2)":"transparent",alignItems:"center",transition:"background 0.15s"}}>
            ...
            <div style={{fontSize:11,color:"#94a3b8"}}>
              ...
              {(r.items||[]).length>2&&<div style={{color:"#475569"}}>+{(r.items||[]).length-2} mas</div>}
            </div>
      ...
      {showEditor&&(
        <ReciboEditor
          recibo={editingRecibo} clients={clients} concepts={concepts} company={company}
          onSave={async(r,client,finalItems)=>{
            await onSaveRecibo(r);
            if(onUpdateClient&&client){
              const ec=clients.find(c=>c.id===r.clientId);
              if(ec){const eids=(ec.items||[]).map(i=>i.id);const ni=finalItems.filter(i=>!eids.includes(i.id));if(ni.length>0)await onUpdateClient({...ec,items:[...(ec.items||[]),...ni]});}
            }
      ...
      {showCopyAll&&(
        <div style={S.overlay}>
          <div style={{...S.modal,maxWidth:440}}>
            <div style={S.modalHead}><span style={S.modalTitle}>Copiar todos los recibos</span><button onClick={()=>setShowCopyAll(false)} style={S.xBtn}>X</button></div>
            <div style={{background:"rgba(15,23,42,0.5)",borderRadius:8,padding:14,marginBottom:16}}>
              <div style={{fontSize:13,color:"#f1f5f9",fontWeight:700,marginBottom:8}}>{filtered.length} recibo(s) de {MONTHS[filterMonth]} {filterYear}:</div>
              {filtered.map(r=><div key={r.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"4px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}><span style={{color:"#94a3b8"}}>{r.clientName}</span><span style={{color:"#6ee7b7",fontWeight:700}}>{fmt(r.total)}</span></div>)}
              <div style={{display:"flex",justifyContent:"space-between",marginTop:8,fontWeight:800,fontSize:13}}><span style={{color:"#64748b"}}>Total</span><span style={{color:"#6ee7b7"}}>{fmt(totalMes)}</span></div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
              <div style={S.field}><label style={S.lbl}>Mes destino</label><select value={toMonth} onChange={e=>setToMonth(Number(e.target.value))} style={S.inp}>{MONTHS.map((m,i)=><option key={i} value={i}>{m}</option>)}</select></div>
              <div style={S.field}><label style={S.lbl}>Anio destino</label><select value={toYear} onChange={e=>setToYear(Number(e.target.value))} style={S.inp}>{[2024,2025,2026,2027].map(y=><option key={y}>{y}</option>)}</select></div>
            </div>
            <div style={{fontSize:12,color:"#64748b",marginBottom:16}}>Se copiaran como pendientes. Podras editarlos uno por uno.</div>
            <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
              <button onClick={()=>setShowCopyAll(false)} style={{...S.btn,...S.btnGhost}}>Cancelar</button>
              <button onClick={handleCopyAll} style={{...S.btn,...S.btnPrimary}}>Copiar a {MONTHS[toMonth]} {toYear}</button>
            </div>
          </div>
        </div>
      )}
...
function MovimientosView({ clients, status, concepts, conceptsConfig, company, onUpdateClient }) {
  const [filterMonth,setFilterMonth] = useState("all");
  const [filterYear,setFilterYear] = useState(new Date().getFullYear());
  ...
  const allItems = useMemo(()=>{
    ...
    clients.forEach(c=>{
      (c.items||[]).forEach(i=>{
        ...
        rows.push({...i,clientName:c.name,clientId:c.id,esGravado:cfg[i.concept]!=="no_gravado"});
      });
    });
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <h2 style={{margin:0,fontSize:20,fontWeight:900,color:"#f1f5f9"}}>{isPending?"Pendientes de cobro":"Cobrado"}</h2>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
...
function GastosView({ gastos, gastoCats, clients, onSave, onDelete, onSaveCats }) {
  const now = new Date();
  const [filterMonth,setFilterMonth] = useState(now.getMonth());
  const [filterYear,setFilterYear] = useState(now.getFullYear());
  ...
  const [f,setF] = useState({concepto:"",categoria:gastoCats[0]||"Otro",monto:"",fecha:today(),estado:"pendiente",tipo:"propio",clienteId:"",clienteNombre:""});
  const set = k=>e=>setF({...f,[k]:e.target.value});
  ...
  const totalPagado = filtered.filter(g=>g.estado==="pagado").reduce((s,g)=>s+(g.monto||0),0);
  const totalPendiente = filtered.filter(g=>g.estado==="pendiente").reduce((s,g)=>s+(g.monto||0),0);
  const totalMes = filtered.reduce((s,g)=>s+(g.monto||0),0);
  const propios = filtered.filter(g=>g.tipo==="propio");
  const deClientes = filtered.filter(g=>g.tipo==="cliente");

  const handleSave = async()=>{
    if(!f.concepto||!f.monto)return alert("Completa concepto y monto.");
    const g={...f,id:editingGasto?editingGasto.id:uid(),monto:parseFloat(f.monto),mes:new Date(f.fecha).getMonth(),anio:new Date(f.fecha).getFullYear()};
    await onSave(g);
    setShowForm(false);setEditingGasto(null);
    setF({concepto:"",categoria:gastoCats[0]||"Otro",monto:"",fecha:today(),estado:"pendiente",tipo:"propio",clienteId:"",clienteNombre:""});
  };
  const startEdit = g=>{setEditingGasto(g);setF({...g,monto:String(g.monto)});setShowForm(true);};
  const toggleEstado = async g=>await onSave({...g,estado:g.estado==="pagado"?"pendiente":"pagado"});

  const CatManager = ()=>{
    const [cats,setCats] = useState([...gastoCats]);
    const [newCat,setNewCat] = useState("");
    return (
      <div style={S.overlay}>
        <div style={{...S.modal,maxWidth:380}}>
          <div style={S.modalHead}><span style={S.modalTitle}>Categorias de Gastos</span><button onClick={()=>setShowCats(false)} style={S.xBtn}>X</button></div>
          <div style={{marginBottom:14}}>
            {cats.map((c,i)=>(
              <div key={c} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
                <span style={{flex:1,fontSize:13,color:"#e2e8f0"}}>{c}</span>
                <button onClick={()=>{if(cats.length>1)setCats(cats.filter(x=>x!==c));}} style={{...S.iconBtn,color:"#fca5a5",fontSize:12}}>X</button>
              </div>
            ))}
          </div>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <input value={newCat} onChange={e=>setNewCat(e.target.value)} onKeyDown={e=>e.key==="Enter"&&newCat.trim()&&!cats.includes(newCat.trim())&&(setCats([...cats,newCat.trim()]),setNewCat(""))} placeholder="Nueva categoria..." style={{...S.inp,flex:1}}/>
            <button onClick={()=>{if(newCat.trim()&&!cats.includes(newCat.trim())){setCats([...cats,newCat.trim()]);setNewCat("");}}} style={{...S.btn,...S.btnPrimary,padding:"8px 14px"}}>+</button>
          </div>
          <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
            <button onClick={()=>setShowCats(false)} style={{...S.btn,...S.btnGhost}}>Cancelar</button>
            <button onClick={async()=>{await onSaveCats(cats);setShowCats(false);}} style={{...S.btn,...S.btnPrimary}}>Guardar</button>
          </div>
        </div>
      </div>
    );
  ...
  const GastoRow = ({g,idx})=>(
    <div style={{display:"grid",gridTemplateColumns:"90px 1fr 2fr 100px 100px 110px 70px",padding:"9px 14px",borderBottom:"1px solid rgba(255,255,255,0.03)",background:idx%2===0?"rgba(15,23,42,0.2)":"transparent",alignItems:"center",gap:8}}>
      <span style={{...S.badge,background:g.tipo==="propio"?"rgba(147,197,253,0.12)":"rgba(253,230,138,0.12)",color:g.tipo==="propio"?"#93c5fd":"#fde68a",fontSize:10,textAlign:"center"}}>{g.tipo==="propio"?"Propio":"Cliente"}</span>
      <span style={{...S.ctag,background:"rgba(148,163,184,0.15)",color:"#94a3b8",fontSize:10}}>{g.categoria}</span>
      <div>
        <div style={{fontSize:12,color:"#e2e8f0",fontWeight:600}}>{g.concepto}</div>
        {g.clienteNombre&&<div style={{fontSize:10,color:"#475569"}}>{g.clienteNombre}</div>}
      </div>
      <span style={{fontWeight:700,color:"#fca5a5",fontSize:12}}>{fmt(g.monto)}</span>
      <span style={{fontSize:11,color:"#475569"}}>{g.fecha}</span>
      <button onClick={()=>toggleEstado(g)} style={{...S.badge,...(g.estado==="pagado"?S.bPaid:S.bPend),fontSize:10}}>
        {g.estado==="pagado"?"Pagado":"Pendiente"}
      </button>
      <div style={{display:"flex",gap:4}}>
        <button onClick={()=>startEdit(g)} style={{...S.iconBtn,color:"#fde68a",fontSize:12}}>E</button>
        <button onClick={()=>{if(window.confirm("Eliminar?"))onDelete(g.id);}} style={{...S.iconBtn,color:"#fca5a5",fontSize:12}}>X</button>
      </div>
    </div>
  );
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          ...
          <select value={filterYear} onChange={e=>setFilterYear(Number(e.target.value))} style={{...S.inp,padding:"6px 10px",fontSize:12,width:"auto"}}>{[2024,2025,2026,2027].map(y=><option key={y}>{y}</option>)}</select>
          <button onClick={()=>setShowCats(true)} style={{...S.btn,...S.btnGhost,fontSize:12,padding:"6px 12px"}}>Categorias</button>
          <button onClick={()=>{setEditingGasto(null);setF({concepto:"",categoria:gastoCats[0]||"Otro",monto:"",fecha:today(),estado:"pendiente",tipo:"propio",clienteId:"",clienteNombre:""});setShowForm(!showForm);}} style={{...S.btn,...S.btnPrimary}}>+ Nuevo Gasto</button>
        </div>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:16}}>
        {[["Total mes",fmt(totalMes),"#fca5a5"],["Pagado",fmt(totalPagado),"#6ee7b7"],["Pendiente",fmt(totalPendiente),"#fde68a"],["Movimientos",filtered.length,"#94a3b8"]].map(([l,v,c])=>(
          <div key={l} style={{...S.card,padding:"12px 14px"}}>
      ...
      {showForm&&(
        <div style={{...S.card,marginBottom:16}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div style={S.field}><label style={S.lbl}>Tipo</label>
              <select value={f.tipo} onChange={set("tipo")} style={S.inp}>
                <option value="propio">Gasto propio</option>
                <option value="cliente">Gasto de cliente</option>
              </select>
            </div>
            <div style={S.field}><label style={S.lbl}>Categoria</label>
              <select value={f.categoria} onChange={set("categoria")} style={S.inp}>{gastoCats.map(c=><option key={c}>{c}</option>)}</select>
            </div>
            {f.tipo==="cliente"&&(
              <div style={{...S.field,gridColumn:"1/-1"}}><label style={S.lbl}>Cliente</label>
                <select value={f.clienteId} onChange={e=>{const c=clients.find(x=>x.id===e.target.value);setF({...f,clienteId:e.target.value,clienteNombre:c?c.name:"" });}} style={S.inp}>
                  <option value="">Seleccionar cliente...</option>
                  {[...clients].sort((a,b)=>(a.name||"").localeCompare(b.name||"")).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </div>
            )}
            <div style={{...S.field,gridColumn:"1/-1"}}><label style={S.lbl}>Concepto / Descripcion</label>
              <input value={f.concepto} onChange={set("concepto")} placeholder="Ej. Alquiler oficina junio" style={S.inp}/>
            </div>
            <div style={S.field}><label style={S.lbl}>Monto ($)</label>
              <input type="number" value={f.monto} onChange={set("monto")} placeholder="0.00" style={S.inp}/>
            </div>
            <div style={S.field}><label style={S.lbl}>Fecha</label>
              <input type="date" value={f.fecha} onChange={set("fecha")} style={S.inp}/>
            </div>
            <div style={S.field}><label style={S.lbl}>Estado</label>
              <select value={f.estado} onChange={set("estado")} style={S.inp}>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
            <button onClick={()=>{setShowForm(false);setEditingGasto(null);}} style={{...S.btn,...S.btnGhost}}>Cancelar</button>
            <button onClick={handleSave} style={{...S.btn,...S.btnPrimary}}>{editingGasto?"Guardar cambios":"+ Agregar gasto"}</button>
          </div>
        </div>
      )}

      {filtered.length===0&&!showForm&&<div style={S.empty}>Sin gastos en {MONTHS[filterMonth]} {filterYear}</div>}

      {propios.length>0&&(
        <div style={{...S.card,padding:0,overflow:"hidden",marginBottom:12}}>
          <div style={{padding:"8px 14px",background:"rgba(147,197,253,0.08)",borderBottom:"1px solid rgba(147,197,253,0.12)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:11,fontWeight:700,color:"#93c5fd",textTransform:"uppercase",letterSpacing:"0.06em"}}>Gastos propios ({propios.length})</span>
            <span style={{fontWeight:800,color:"#93c5fd",fontSize:13}}>{fmt(propios.reduce((s,g)=>s+(g.monto||0),0))}</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"90px 1fr 2fr 100px 100px 110px 70px",padding:"7px 14px",background:"rgba(15,23,42,0.8)",borderBottom:"1px solid rgba(255,255,255,0.06)",gap:8}}>
            {["Tipo","Cat.","Concepto","Monto","Fecha","Estado",""].map(h=><div key={h} style={{fontSize:10,color:"#475569",textTransform:"uppercase",letterSpacing:"0.06em",fontWeight:700}}>{h}</div>)}
          </div>
          {propios.map((g,idx)=><GastoRow key={g.id} g={g} idx={idx}/>)}
        </div>
      )}
      ...
      {deClientes.length>0&&(
        <div style={{...S.card,padding:0,overflow:"hidden"}}>
          <div style={{padding:"8px 14px",background:"rgba(253,230,138,0.06)",borderBottom:"1px solid rgba(253,230,138,0.1)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:11,fontWeight:700,color:"#fde68a",textTransform:"uppercase",letterSpacing:"0.06em"}}>Gastos de clientes ({deClientes.length})</span>
            <span style={{fontWeight:800,color:"#fde68a",fontSize:13}}>{fmt(deClientes.reduce((s,g)=>s+(g.monto||0),0))}</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"90px 1fr 2fr 100px 100px 110px 70px",padding:"7px 14px",background:"rgba(15,23,42,0.8)",borderBottom:"1px solid rgba(255,255,255,0.06)",gap:8}}>
            {["Tipo","Cat.","Concepto","Monto","Fecha","Estado",""].map(h=><div key={h} style={{fontSize:10,color:"#475569",textTransform:"uppercase",letterSpacing:"0.06em",fontWeight:700}}>{h}</div>)}
          </div>
          {deClientes.map((g,idx)=><GastoRow key={g.id} g={g} idx={idx}/>)}
        </div>
      )}
...
function ChartView({ clients, concepts, conceptsConfig, gastos }) {
  ...
  const monthlyData = useMemo(()=>MONTHS_SHORT.map((m,mi)=>{
    const row={month:m};let totalIng=0;let totalGasto=0;
    gravados.forEach(c=>{
      let sum=0;
      ...
      if(sum>0)row[c]=sum;totalIng+=sum;
    });
    (gastos||[]).forEach(g=>{
      const d=new Date(g.fecha);
      if(d.getFullYear()===year&&d.getMonth()===mi&&g.estado==="pagado")totalGasto+=g.monto||0;
    });
    row.Ingresos=totalIng;row.Gastos=totalGasto;row.Total=totalIng;
    return row;
  }),[clients,gastos,year,concepts]);
  ...
  const curIng = monthlyData[currentMonth]?.Ingresos||0;
  const prevIng = monthlyData[prevMonth]?.Ingresos||0;
  const curGasto = monthlyData[currentMonth]?.Gastos||0;
  const prevGasto = monthlyData[prevMonth]?.Gastos||0;
  const varIng = prevIng>0?((curIng-prevIng)/prevIng*100):null;
  const varGasto = prevGasto>0?((curGasto-prevGasto)/prevGasto*100):null;
  const resultado = curIng - curGasto;

  const Tip = ({active:a,payload,label})=>a&&payload&&payload.length?(<div style={{background:"#1e293b",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"10px 14px"}}><div style={{fontWeight:700,marginBottom:6,color:"#f1f5f9",fontSize:13}}>{label}</div>{payload.map(p=><div key={p.name} style={{display:"flex",justifyContent:"space-between",gap:14,fontSize:12}}><span style={{color:p.color||getColor(p.name,concepts)}}>{p.name}</span><span style={{fontWeight:700,color:"#f1f5f9"}}>{fmt(p.value)}</span></div>)}</div>):null;

  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <h2 style={{margin:0,fontSize:20,fontWeight:900,color:"#f1f5f9"}}>Analisis de Ingresos</h2>
        <div style={{display:"flex",gap:8}}>
          <select value={year} onChange={e=>setYear(Number(e.target.value))} style={{...S.inp,padding:"7px 12px",fontSize:13,width:"auto"}}>{[2024,2025,2026,2027].map(y=><option key={y}>{y}</option>)}</select>
          ...
          <button onClick={()=>setChartType("line")} style={{...S.btn,...(chartType==="line"?S.btnPrimary:S.btnGhost),padding:"7px 14px",fontSize:13}}>Lineas</button>
          <button onClick={()=>setChartType("ingvsgasto")} style={{...S.btn,...(chartType==="ingvsgasto"?S.btnPrimary:S.btnGhost),padding:"7px 14px",fontSize:13}}>Ing vs Gasto</button>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:10,marginBottom:16}}>
        <div style={{...S.card,padding:"12px 14px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div><div style={{fontWeight:800,fontSize:17,color:"#6ee7b7"}}>{fmt(curIng)}</div><div style={{fontSize:10,color:"#475569",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:3}}>Ingresos {MONTHS_SHORT[currentMonth]}</div></div>
            {varIng!==null&&<div style={{fontSize:11,fontWeight:700,color:varIng>=0?"#6ee7b7":"#fca5a5",background:varIng>=0?"rgba(110,231,183,0.1)":"rgba(252,165,165,0.1)",padding:"2px 7px",borderRadius:12}}>{varIng>=0?"+":""}{varIng.toFixed(1)}%</div>}
          </div>
        </div>
        <div style={{...S.card,padding:"12px 14px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div><div style={{fontWeight:800,fontSize:17,color:"#fca5a5"}}>{fmt(curGasto)}</div><div style={{fontSize:10,color:"#475569",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:3}}>Gastos {MONTHS_SHORT[currentMonth]}</div></div>
            {varGasto!==null&&<div style={{fontSize:11,fontWeight:700,color:varGasto<=0?"#6ee7b7":"#fca5a5",background:varGasto<=0?"rgba(110,231,183,0.1)":"rgba(252,165,165,0.1)",padding:"2px 7px",borderRadius:12}}>{varGasto>=0?"+":""}{varGasto.toFixed(1)}%</div>}
          </div>
        </div>
        <div style={{...S.card,padding:"12px 14px"}}>
          <div style={{fontWeight:800,fontSize:17,color:resultado>=0?"#6ee7b7":"#fca5a5"}}>{fmt(resultado)}</div>
          <div style={{fontSize:10,color:"#475569",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:3}}>Resultado {MONTHS_SHORT[currentMonth]}</div>
        </div>
      </div>

      <div style={{...S.card,marginBottom:20}}>
        <div style={{fontSize:12,color:"#64748b",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4,fontWeight:700}}>
          {chartType==="ingvsgasto"?"Ingresos vs Gastos":"Ingresos cobrados"} {year}
        </div>
        <div style={{fontSize:11,color:"#475569",marginBottom:14}}>
          {chartType==="ingvsgasto"?"Comparativa mensual":"Solo conceptos gravados ("+gravados.length+" de "+concepts.length+")"}
        </div>
        {!monthlyData.some(r=>r.Total>0)&&!monthlyData.some(r=>r.Gastos>0)?<div style={S.empty}>Sin datos para {year}</div>:(
          <ResponsiveContainer width="100%" height={280}>
            {chartType==="ingvsgasto"?(
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:12}}>
        {totals.map(({concept,color,total})=>(
          <div key={concept} style={{...S.card,padding:"14px 16px"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{width:9,height:9,borderRadius:"50%",background:color,flexShrink:0}}/><span style={{fontSize:12,color:"#94a3b8"}}>{concept}</span></div>
            ...
            <div style={{height:3,background:"rgba(255,255,255,0.05)",borderRadius:99,marginTop:8}}><div style={{height:3,borderRadius:99,background:color,width:(grand>0?(total/grand)*100:0)+"%"}}/></div>
          </div>
        ))}
      </div>
    </div>
...
export default function App() {
  ...
  const [gastoCats,setGastoCats] = useState(DEFAULT_GASTO_CATS);
  ...
  useEffect(()=>{
    ...
    const u6=onSnapshot(doc(db,"users",user.uid,"settings","gastoCats"),s=>{if(s.exists()&&s.data().list)setGastoCats(s.data().list);});
  ...
  const logout=async()=>{await signOut(auth);setClients([]);setView("chart");setSelected(null);};
  ...
  const saveConcepts=async(l,cfg)=>{setConcepts(l);setConceptsConfig(cfg||DEFAULT_CONCEPTS_CONFIG);await setDoc(doc(db,"users",user.uid,"settings","concepts"),{list:l,config:cfg||DEFAULT_CONCEPTS_CONFIG});setShowConcepts(false);};
  const saveRecibo=async r=>await setDoc(doc(db,"users",user.uid,"recibos",r.id),r);
  const saveGasto=async g=>await setDoc(doc(db,"users",user.uid,"gastos",g.id),g);
  const deleteGasto=async id=>await deleteDoc(doc(db,"users",user.uid,"gastos",id));
  const saveGastoCats=async l=>{setGastoCats(l);await setDoc(doc(db,"users",user.uid,"settings","gastoCats"),{list:l});};
  ...
  const totalGastosDelMes=gastos.filter(g=>g.mes===new Date().getMonth()&&g.anio===new Date().getFullYear()).reduce((s,g)=>s+(g.monto||0),0);
    ...
    <div style={S.root}>
      ...
      <nav style={S.nav}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
          ...
          <div><div style={{fontWeight:800,fontSize:14,color:"#f1f5f9"}}>ClientesPro</div><div style={{fontSize:9,color:"#475569",textTransform:"uppercase",letterSpacing:"0.1em"}}>v7.0</div></div>
        </div>
        ...
        <div style={{margin:"14px 0",borderTop:"1px solid rgba(255,255,255,0.05)",paddingTop:14}}>
          {[["Cobrado",fmt(allPaid),"#6ee7b7"],["Pendiente",fmt(allPend),"#fca5a5"],["Gastos mes",fmt(totalGastosDelMes),"#fde68a"],["Clientes",clients.length,"#94a3b8"]].map(([l,v,c])=>(
            <div key={l} style={{display:"flex",justifyContent:"space-between",marginBottom:5,fontSize:11}}><span style={{color:"#475569"}}>{l}</span><span style={{fontWeight:700,color:c}}>{v}</span></div>
          ))}
        </div>
        <div style={{marginTop:"auto",display:"flex",flexDirection:"column",gap:6}}>
          ...
          <button onClick={()=>{const rows=[["Cliente","CUIT","Concepto","Descripcion","Monto","Fecha","Estado"]];clients.forEach(c=>(c.items||[]).forEach(i=>rows.push([c.name,c.cuit||"",i.concept,i.description,i.amount,i.date,i.status])));const csv=rows.map(r=>r.map(v=>"\""+v+"\"").join(",")).join("\n");const a=document.createElement("a");a.href=URL.createObjectURL(new Blob(["\uFEFF"+csv],{type:"text/csv;charset=utf-8;"}));a.download="clientes_pro.csv";a.click();}} style={{...S.btn,...S.btnGhost,fontSize:11,padding:"6px",width:"100%"}}>Exportar CSV</button>
      ...
      <div style={S.main}>
          ...
          <>
            ...
            <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
              <div style={{flex:1,minWidth:180,display:"flex",alignItems:"center",background:"rgba(30,41,59,0.6)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,padding:"0 12px"}}>
                <span style={{opacity:0.3,marginRight:8,fontSize:12}}>B</span>
        ...
        {!dbLoading&&view==="gastos"&&<GastosView gastos={gastos} gastoCats={gastoCats} clients={clients} onSave={saveGasto} onDelete={deleteGasto} onSaveCats={saveGastoCats}/>}
        ...
        {!dbLoading&&view==="chart"&&<ChartView clients={clients} concepts={concepts} conceptsConfig={conceptsConfig} gastos={gastos}/>}
      </div>
